...
- `outgoing`: truncated-узлы, ранжированные по `in_kzt`, затем `in_deg` — «сеть обрывается там, куда пришло
  больше всего денег: запросите исходящие». (`seed_reach` насыщается на 7 и выводил вперёд узлы с входом
  15 тыс. ₸.)
