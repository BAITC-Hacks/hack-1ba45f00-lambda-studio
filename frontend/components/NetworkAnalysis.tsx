...
export default function NetworkAnalysis() {
  ...
  const paintNode = (raw: any, context: CanvasRenderingContext2D, scale: number) => {
    ...
    const radius = compact ? (core ? 4 + node.priority * 5 : node.is_seed ? 3.5 : 2.5) : showcaseCore ? 17 + node.priority * 9 : showcaseMid ? 7 : node.is_seed ? 3 : core ? 5 + node.priority * 9 : Math.max(2, 2 + node.priority * 3); const color = dead ? '#3c4240' : node.is_seed ? '#d9dfdc' : colorMode === 'cluster' ? `hsl(${(node.cluster * 137.5) % 360} 45% 65%)` : colors[node.role];
