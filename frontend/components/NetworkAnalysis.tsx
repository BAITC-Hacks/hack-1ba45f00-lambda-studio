...
export default function NetworkAnalysis() {
  ...
  const shown = useMemo(() => graph.nodes.filter(node => (showAll ? (!ego || ego.nodes.includes(node.id) || !!detail?.upstream_seeds.includes(node.id)) : overview ? priorityIds.has(node.id) : compactIds.has(node.id)) && (!coreOnly || node.in_core) && (role === 'all' || node.role === role) && (cluster === 'all' || node.cluster === Number(cluster))).map(node => {
    ...
    // «Топ-30 + соседи» раскладывается сам по себе (свободная симуляция от координат бэкенда):
    // в глобальной раскладке его узлы разбросаны по всему полотну и связаны нитями через весь экран.
    if (overview && !showAll) return { ...node };
    return showAll ? { ...node, fx: node.x, fy: node.y } : { ...node, x: position?.x ?? 0, fx: position?.x ?? 0, fy: position?.y ?? 0 };
  ...
  // «Топ-30 + соседи»: сильнее отталкивание и длиннее связи — узлам нужно место
  useEffect(() => {
    const fg = graphRef.current;
    if (!fg || !overview || showAll) return;
    fg.d3Force('charge')?.strength(-160).distanceMax(400);
    fg.d3Force('link')?.distance(55);
    fg.d3ReheatSimulation();
  }, [overview, showAll, useGraphData]);
  ...
  const paintNode = (raw: any, context: CanvasRenderingContext2D, scale: number) => {
    ...
    const compact = overview && !showAll;   // «Топ-30 + соседи»: мелкие узлы и слабое свечение, чтобы не слипались
    const radius = compact ? (core ? 4 + node.priority * 5 : node.is_seed ? 3.5 : 2.5) : showcaseCore ? 17 + node.priority * 9 : showcaseMid ? 7 : node.is_seed ? 3 : core ? 5 + node.priority * 9 : Math.max(2, 2 + node.priority * 3); const color = dead ? '#3c4240' : node.is_seed && node.role === 'peripheral' ? '#d9dfdc' : colorMode === 'cluster' ? `hsl(${(node.cluster * 137.5) % 360} 45% 65%)` : colors[node.role];
    context.save(); if (core && !dead) { const glowK = compact ? 1.7 : 2.8; const glow = context.createRadialGradient(x, y, radius * .2, x, y, radius * glowK); glow.addColorStop(0, color); glow.addColorStop(1, 'transparent'); context.fillStyle = glow; context.fillRect(x - radius * glowK, y - radius * glowK, radius * glowK * 2, radius * glowK * 2); }
