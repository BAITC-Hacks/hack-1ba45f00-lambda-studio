...
### Интерфейс: пять вкладок

Все выводы на экране — гипотезы для проверки человеком, а не утверждения о виновности.

1. **«Анализ сети».**
   - **Обзор путей** — главный вид для демо: сверху курьеры («грибы»), ниже реальные цепочки переводов шаг
     за шагом, внизу пять узлов с наивысшим приоритетом проверки. Курьер, который сам получил роль
     (например, координатор), окрашен в цвет роли.
   - **Топ-30 + соседи** и **Весь граф** — силовая раскладка (`react-force-graph`): связанные узлы
     стягиваются в группы; «Ядро» подсвечивает кольцо из 85 узлов, где деньги могут вернуться к отправителю.
   - **Поиск** по полному gid или его части и **карточка узла**: роль, «роль устойчива в K из 50 вариантов
     порогов», блок «Почему эта роль» (правила ✓/✗ с фактическими значениями и порогами), суммы входа и
     выхода, плательщики и получатели.
2. **«Кого проверять»** — 30 узлов по убыванию `priority_score` с обоснованием на числах; клик открывает узел.
3. **«Лестница денег»** — Санкей по коленам 0–4: толщина ленты равна сумме; сноска показывает, какая доля
   денег ходит по кругу или вбок (в основном через ядро-кольцо).
4. **«Блокировка»** — сравнение стратегий (план блокировки, топ проверки, топ по обороту, случайные узлы) для
   5 / 10 / 20 узлов по метрике `drop_reach_pct` — на сколько процентов падает поток денег курьеров;
   «Смоделировать на графе» вызывает `POST /api/block`. Подпись: проверять и блокировать — разные задачи.
5. **«Справка ИИ»** — справка по делу и вопросы аналитика. Модель отвечает через инструменты над
   посчитанными данными (function calling, `mycelium/ai/tools.py`), а `mycelium/ai/verify.py` сверяет каждый
   gid и каждое число с результатами: ответ получает значок «✓ сверено с графом» или честный список
   расхождений. Вопросы о виновности модель не получает — ответ фиксированный и собирается из данных.
