...
Необязательны. Для живого ИИ-аналитика скопируйте `.env.example` в `.env` и заполните:
...
Без ключа `GET /api/health` отвечает `"ai_enabled": false`, а ИИ-вкладка показывает сохранённую справку.
...
| 3 | `python -m pytest -q` | **16 passed** — форма всех ответов API совпадает с `docs/mock` |
...
При N = 20 план даёт −48,2 % / −63,5 %. Топ проверки ищет тех, кто организует (координаторы и точки
